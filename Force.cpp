...
vector<int> Force::execute() const
{
    ...
    return execute(var_of_idx);
}

/**
 * Specify a starting order.
 */
vector<int> Force::execute(vector<int> var_of_idx) const
{
	size_t num_vars = _graph.num_vertices();
	assert(var_of_idx.size() == num_vars);

	vector<int> idx_of_var(num_vars, -1);
	construct_lookup(idx_of_var, var_of_idx);

	long best_sum_spans = compute_sum_of_spans(idx_of_var);
	cout << best_sum_spans << endl;
